...
class Event extends REST_Controller{
	...
	function list_post () {
		...
		$data = $this->live_events_model->list_events($filters, $date);
    ...
    function get_by_id_get () {
		...
		$data = $this->live_events_model->get_by_id($id);
	...
	function event_subscription_post(){
		$data = $this->post();
		$row = $this->live_events_model->subscription_checkout($data);
	...
	function event_subscription_mobile_post(){
		$data = $this->post();
		$row = $this->live_events_model->subscription_mobile_checkout($data);
	...
	function buy_event_by_stored_cc_post(){
		$data = $this->post();
		$row = $this->live_events_model->buy_event_by_stored_cc($data);
		$this->response($row, 200);
	}
