var pay = null;
uvodApp.controller('EventsController', function($scope, $interval, $location, globalFactory, User, AuthService, $log, $window) {
    pay = $scope;
    $scope.user = User;
    $scope.filterList = {};
    $window.scroll(0, 0);
    ...
    $scope.getEvents = function() {
        ...
        globalFactory.getEvents($scope.filterList).then(function(data) {
            $scope.events = data.content.entries;
            $scope.mainEvent = $scope.events[0];
            ...
            for (evt in $scope.events) {
                $scope.events[evt].tickets = $scope.events[evt].tickets.sort(function(itemA, itemB) {
    ...
    function getTimeRemaining(key, endtime) {
        ...
        $scope.events[key].days = (days >= 10) ? days : '0' + days;
        $scope.events[key].hours = (hours >= 10) ? hours : '0' + hours;
        $scope.events[key].minutes = (minutes >= 10) ? minutes : '0' + minutes;
        $scope.events[key].seconds = (seconds >= 10) ? seconds : '0' + seconds;
        $scope.events[key].total = t;
    }
