var payItem = null;
uvodApp.controller('EventItemController', function($scope, $interval, $routeParams, globalFactory, User, AuthService, $window, $location, $log, $http) {
    payItem = $scope;
    $scope.lineup = 1;
    $window.scroll(0, 0);
    ...
    globalFactory.getEventById($routeParams.eventId).then(function(data) {
        $scope.event = data[0];
        $scope.eventType = $scope.event.categories ? $scope.event.categories[0].name : 'commerce_free_media';
       
        $scope.mainImage = $scope.event.PosterH;
        initializeClock($scope.event.event_date);
        // console.log(($scope.event));
        $scope.eventIsLive = $scope.event.live_now;
